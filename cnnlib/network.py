...
class CNN(object):
    ...
    def model(self):
        ...
        conv1 = tf.nn.relu(
            tf.nn.bias_add(tf.nn.conv2d(input=x, filters=wc1, strides=[1, 1, 1, 1], padding='SAME'), bc1))
        ...
        conv2 = tf.nn.relu(
            tf.nn.bias_add(tf.nn.conv2d(input=conv1, filters=wc2, strides=[1, 1, 1, 1], padding='SAME'), bc2))
        ...
        conv3 = tf.nn.relu(
            tf.nn.bias_add(tf.nn.conv2d(input=conv2, filters=wc3, strides=[1, 1, 1, 1], padding='SAME'), bc3))
